...
##    Base cases: set A[i][j][0] to the edge cost if there is an edge
    for j in inNodes:
        for edge in inNodes[j]:
            i = edge[0]
            solved[0][i][j] = edge[1]
    ...
    for k in range (1, numNodes+2):
        for i in range (1, numNodes+2):
            for j in range (1, numNodes+2):
                choice1 = solved[k-1][i][j]
                choice2 = solved[k-1][i][k] + solved[k-1][k][j]
                solved[k][i][j] = min (choice1, choice2)
            if solved[k][i][i] < 0:
                print "Found a negative cycle"
                return
        if k%10 == 0:
            print "Still going, k is ", k
##        print "---------------------"
##        print " Finished iteration k = ", k
        

    for i in range (1, numNodes+2):
        if solved[numNodes+1][i][i] < 0:
            negativeCycle = True
        currentShortest = solved[numNodes+1][i][0]
        for j in range (1, numNodes+2):
            if solved[numNodes+1][i][j] < currentShortest:
                currentShortest = solved[numNodes+1][i][j]
        shortestPerNode[i] = currentShortest
